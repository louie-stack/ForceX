...
/**
 * The hero stage. One cloud of ~24k points cycles through four targets:
 * a scattered cloud on load, the word VERIFIED sampled from the page font,
 * a cube lattice, and a sphere. A caption under the stage changes in sync
 * with the shape, so the sentence completes differently each phase.
 */
type Phase = "text" | "cube" | "sphere";

const PHASES: { phase: Phase; hold: number; caption: string }[] = [
  { phase: "text", hold: 5.2, caption: "before it is displayed." },
  { phase: "cube", hold: 3.4, caption: "Every block, reconciled." },
  { phase: "sphere", hold: 3.4, caption: "Cross-checked against the node." },
];
...
const VERT = /* glsl */ `
uniform float uTime;
uniform vec4 uW;        // cloud, cube, text, sphere
...
void main() {
  ...
  // Outward puff mid-transition so shapes explode and re-form.
...
function sampleWord(word: string, family: string, worldWidth: number): Float32Array {
  const W = 1600;
  const H = 360;
...
export function HeroStage({ children }: { children?: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const count = n * n * n;
    const half = isMobile ? 2.4 : 3.1;
    const R = isMobile ? 2.7 : 3.9;
    ...
    // Sample the word once the page font is available.
    const fillText = async () => {
      ...
      const visW = 2 * camera.position.z * Math.tan(THREE.MathUtils.degToRad(19)) * camera.aspect;
      const worldW = Math.min(15.5, visW * (isMobile ? 0.92 : 0.8));
      const pts = sampleWord(WORD, `"${family}", sans-serif`, worldW);
      const m = pts.length / 2;
      const attr = geo.getAttribute("aText") as THREE.BufferAttribute;
      const arr = attr.array as Float32Array;
      for (let k = 0; k < count; k++) {
        const j = (k * 7919) % m;
        arr[k * 3] = pts[j * 2] + (Math.random() - 0.5) * 0.08;
        arr[k * 3 + 1] = pts[j * 2 + 1] + (Math.random() - 0.5) * 0.08;
        arr[k * 3 + 2] = (Math.random() - 0.5) * 0.5;
      }
    ...
    const resize = () => {
      ...
      group.position.y = w < 760 ? 1.6 : 0.4;
    };
    resize();
    ...
    const setCaption = (txt: string, immediate = false) => {
      ...
      if (immediate || reduce || chars.length === 0) {
        const next = build();
        gsap.fromTo(next, { yPercent: 110, opacity: 0 }, { yPercent: 0, opacity: 1, duration: 0.9, ease: "power4.out", stagger: 0.018 });
        return;
      }
      gsap.to(chars, {
        yPercent: -110,
        ...
        onComplete: () => {
          const next = build();
          gsap.fromTo(next, { yPercent: 110, opacity: 0 }, { yPercent: 0, opacity: 1, duration: 0.8, ease: "power4.out", stagger: 0.016 });
        },
      });
    };
    ...
    const sizeFor = (ph: Phase) => (ph === "text" ? (isMobile ? 3.4 : 1.9) : isMobile ? 3.2 : 2.4);
    ...
    const render = () => {
      ...
      group.rotation.y = rot.y * Math.sin(spin) + mouse.x * (0.12 + rot.y * 0.3);
      group.rotation.x = rot.x * 0.6 + mouse.y * (0.08 + rot.x * 0.25);
